...
const failure: HeaderFailure = (message) =>
...
function resolveUrl(target: URL, url: string): URL {
  if (url.startsWith("/")) return buildUrl(target, url, undefined);
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  return {
    ...
    async request(call, signal) {
      if (!isHttpMethod(call.method)) {
        throw new GatewayError(
          "INTERNAL",
          `Authentication request: unsupported method "${String(call.method)}"`,
        );
      }
      ...
      if (hasBody && call.method === "GET") {
        throw new GatewayError(
          "INTERNAL",
          "Authentication request: GET requests cannot carry a body",
        );
      }
